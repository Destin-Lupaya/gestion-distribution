...
router.post('/', async (req, res) => {
    let connection;
    try {
        const { evenement_id, beneficiaire_id, household_id, articles_recus, quantite_recue, agent_distributeur_id, mode_verification, notes_distribution } = req.body;
    ...
    `, [evenement_id, beneficiaire_id]);
        ...
        // Créer l'assistance
        const assistanceId = (0, uuid_1.v4)();
...
router.delete('/:assistanceId', async (req, res) => {
    ...
    `, [assistanceId]);
        ...
        const assistanceData = assistance[0];
        // Mettre à jour le statut dans la liste éligible
        await connection.query(`
...
router.get('/statistiques/evenement/:evenementId', async (req, res) => {
    ...
    `, [evenementId]);
        ...
        res.json({
            ...
            statistiques: {
                total_eligibles: totalEligibles[0].total,
                total_servis: servis[0].total,
                total_non_servis: nonServis[0].total,
                pourcentage_couverture: (servis[0].total / totalEligibles[0].total) * 100,
...
//# sourceMappingURL=assistancesRoutes.js.map