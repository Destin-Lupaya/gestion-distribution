...
router.get('/beneficiaires/:numeroEnregistrement', async (req, res) => {
    try {
      ...
      FROM nutrition_beneficiaires nb
      LEFT JOIN nutrition_rations nr ON nb.id = nr.beneficiaire_id
      WHERE nb.numero_enregistrement = ?`, [numeroEnregistrement]);
        await connection.end();
...
router.get('/distributions/:rationId', async (req, res) => {
    try {
        ...
        const [rows] = await connection.execute(`SELECT * FROM nutrition_distributions 
       WHERE ration_id = ? 
       ORDER BY date_distribution DESC`, [rationId]);
        await connection.end();
...
router.post('/distributions', async (req, res) => {
    try {
        const { ration_id, date_distribution, cycle, quantite, pb, observations } = req.body;
        // Create database connection
        const connection = await promise_1.default.createConnection({
            host: process.env.DB_HOST || 'localhost',
            user: process.env.DB_USER || 'root',
        ...
        // Insert distribution
        await connection.execute(`INSERT INTO nutrition_distributions (
        id, 
        ration_id, 
        date_distribution, 
        cycle, 
