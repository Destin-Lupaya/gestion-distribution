...
router.get('/tonnage-comparison', async (req, res) => {
    try {
        const { startDate, endDate, siteId } = req.query;
        // Validation des paramètres
        if (!startDate || !endDate) {
    ...
    `;
        const waybillParams = [startDate, endDate];
        // Ajouter le filtre de site si spécifié
        if (siteId) {
      ...
      WHERE 
        d.distribution_date BETWEEN ? AND ?
    `;
        const mposParams = [startDate, endDate];
        // Ajouter le filtre de site si spécifié
        if (siteId) {
      ...
      WHERE 
        d.distribution_date BETWEEN ? AND ?
    `;
        const beneficiariesParams = [startDate, endDate];
        // Ajouter le filtre de site si spécifié
        if (siteId) {
...
router.get('/batch-commodity', async (req, res) => {
    try {
